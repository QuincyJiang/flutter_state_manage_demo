...
/// 使用Bloc模式构建页面 可以分离业务逻辑 页面部分只与widget相关 并且可以抛弃StatefulWidget 更优雅更清爽
...
/// 3. 自由组织布局：有了Streams，你就可以完全独立于业务逻辑地去组织你的布局了 可以在App中任何位置触发操作，只需要通过对应的sink传入即可；
///    也可以在任何页面的任何位置来展示数据，只需要监听对应的stream。
...
class BlocWidget extends StatelessWidget{
  ...
  Widget build(BuildContext context) {
    ...
    // context.ancestorWidgetOfExactType() 这个方法的时间复杂度为o(n)
    // 不适合目标父widget与当前widget层级相隔太远
    // 除此之外 不同Bloc之间的交流也是问题 总不能都是通过构造方法传参的方式来实现与其他bloc交互的吧
    // 可以看到 BloC模式 更适合维护某个页面相关的局部数据和状态
    // App层级的数据状态 如果在最顶层Widget写一个AppBloc的话
    // 如果当前页面需要获取App相关的数据 通过buildContext去遍历widget树 性能难免不好
    // 如果是从祖先InheritWidget获取数据 在查找符合指定类型的数据时
    // 可以通过InheritWidget的context.inheritFromWidgetOfExactType()
    // 得益于Flutter Framework的优化 它只有o(1) 的复杂度
    // 所以有没有更好的 管理App 全局状态  并且不同的business 模块还可以方便的相互交互的方式呢 ？
